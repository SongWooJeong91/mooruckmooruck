...
const PlantRegist = () => {
	...
	const onClickSunshine = (e) => {
		...
		// true 이면 그 값을 제외한 나머지만 새로 배열 생성 flase 이면 그 값을 배열에 추가
	...
	useEffect(() => {
		setPlant({ ...plant, sunshine: sunshine.join(',') });
	}, [sunshine]);
	...
	// 이미지 등록 저장 함수
	const imgUploadHandleChange = useCallback(
		(e) => {
			// console.log('이미지 등록 저장 함수에서 plnat 확인', plant);
			...
			formData.append('plantResiImg', e.target.files[0]);
			const config = {
				headers: {
			...
			axios
				.post('http://localhost:3005/plant/upload', formData, config)
				.then((res) => {
					console.log('데이터 처리 결과', res.data);
					setPlant({ ...plant, plantImg: res.data });
					// console.log('파일업로드 plant 값 확인', plant);
				})
				.catch((err) => {
